...
This directory contains the [Docker file](Dockerfile) used to build the `hemaia` Docker container. The container is based on the Ubuntu 24.04 LTS image and comes with all free development tools. The environment is also already configured, such that no additional steps are required to work in the container after installation.
...
To download/push the container, first login to the GitHub container registry:
```shell
...
```shell
$ podman pull ghcr.io/kuleuven-micas/hemaia:main
```
...
```shell
$ podman build -t ghcr.io/kuleuven-micas/hemaia:main -f util/container/Dockerfile .
```

Than you can upload the image to the github
```shell
$ podman pull -t ghcr.io/kuleuven-micas/hemaia:main
```
## Usage
...
```shell
$ podman run -it --rm -v $REPO_TOP:/repo -w /repo ghcr.io/kuleuven-micas/hemaia:main
```
...
/your-repo
    ...
    start_hemaia.sh
```
You could write a start_hemaia.sh file as
```
podman run --rm --workdir /your-gitrepo -it -v /your-gitrepo:/your-gitrepo ghcr.io/kuleuven-micas/hemaia:main
```
Than you could run 
```shell
