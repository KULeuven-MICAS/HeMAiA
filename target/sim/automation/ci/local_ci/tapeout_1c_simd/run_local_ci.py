#!/usr/bin/env python3
"""
HeMAiA CI runner -- ``hemaia_tapeout_1c_simd`` suite
====================================================

Builds and runs the ``task_local_ci.yaml`` next to this script through the shared
:class:`HeMAiASimRunner`, against ``target/rtl/cfg/hemaia_tapeout_1c_simd.hjson``:
4 chiplets, 1x ``snax_versacore_to_simd_cluster`` per chiplet.
