...
* **Step 6 — Summary.** Writes a `simulation_summary_*.md` in this directory.
...
are fixed per script. The `--engine` and `--waveform` knobs are exposed so any
flow can be flipped (e.g. `--engine vcs --waveform 0`).
...
## Full local CI and the xDMA sweep
...
* `../sweep/xdma` — the xDMA op-cost LUT sweep (`run_xdma_sweep.py`,
  `task_vsim_xdma_lut.yaml` + the `gather_xdma_luts.py` / `xdma_lut_report.py`
  post-processing). Run via `make -C target/sim/automation sweep-xdma`.
* `../sweep/ara` — the Ara FP32 op-cost sweep (`run_ara_sweep.py`, `task_ara.yaml`
  ...
  `util/sim/ara_lib.py`). Run via `make -C target/sim/automation sweep-ara`.
