...
snitch_cluster_util_sim_path = subprocess.check_output(
    ["bender", "path", "snitch_cluster"], text=True
).strip()
sys.path.insert(0, os.path.join(snitch_cluster_util_sim_path, "util", "sim"))
...
try:
    chisel_ssm_path = subprocess.check_output(
        ["bender", "path", "chisel-ssm"], text=True
    ).strip()
...
class DataGeneratorBase(ABC):
    ...
    def format_int(self, value: int):
        ...
        variable_name = next(
            (name for name, val in callers_local_vars if val is value), None
        )
    ...
    def format_temporal_bounds_strides(
        self, streamer_name: str, mode_id: int, bounds: list[int], strides: list[int]
    ):
        ...
        self.lines_params += [
            f"int32_t M{mode_id}_{streamer_name}_tb[] = {{{', '.join(map(str, bounds))}}};"
        ]
        self.lines_params += [
            f"int32_t M{mode_id}_{streamer_name}_ts[] = {{{', '.join(map(str, strides))}}};"
        ]
    ...
    def format_spatial_stride(self, streamer_name: str, mode_id: int, stride: int):
        ...
        self.lines_params += [
            f"int32_t M{mode_id}_{streamer_name}_ss[] = {{{stride}}};"
        ]
    ...
    def format_test_samples(
        self, mode_id: int, tensor_name: str, tensor_size: int, nb_test_samples: int
    ):
    ...
    def build_mode(
        ...
        for tensor, size in tests.items():
            self.format_test_samples(
                mode_id, tensor, tensor_size=size, nb_test_samples=NB_TEST_SAMPLES
            )
    ...
    def extend_unroll_factor_to_bankwidth(self, unroll_factor: int, elem_width: int):
        ...
        """
        return (
            math.ceil(unroll_factor * elem_width / BANKWIDTH) * BANKWIDTH // elem_width
        )
