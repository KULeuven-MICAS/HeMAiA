...
int main() {
    if (snrt_cluster_idx() == 1) {  // Set err value for checking
        ...
        tcdm_out = (uint8_t *)(tcdm_baseaddress + delta_local_d8 + 64 * 7 * 7);
        ...
        for (int i = 1; i < 32; i++) {
            ...
            if (snrt_is_dm_core()) {
                ...
                end_cycle = snrt_mcycle();
                printf(
                    "[CONV-%d]-DMA transfer cycle from DMA hardware counter %d  "
                    "for A "
                    "and B\r\n", i, snrt_get_perf_counter(SNRT_PERF_CNT0));
                snrt_reset_perf_counter(SNRT_PERF_CNT0);
                printf(
                    "[CONV-%d]-DMA transfer cycle from mcycle: %d cycles for A "
                    "and B "
            ...
            if (snrt_is_dm_core()) {
                ...
                snrt_reset_perf_counter(SNRT_PERF_CNT0);
                printf(
                    "[CONV-%d]-DMA transfer cycle from mcycle: %d cycles for C "
                    "\r\n", i, end_cycle - start_cycle);
            }
            ...
            if (snrt_cluster_core_idx() == 0) {
                ...
                printf("[CONV-%d]-Compute total cycles from mcycle: %d \r\n",
                       i, end_cycle - start_cycle);
        ...
        if (snrt_cluster_core_idx() == 0) {
            ...
            set_gemmx_streamer_csr(
                ...
                delta_local_d8 + 64 * 7 * 7 + 32 * 64, delta_local_b_fc, delta_local_d8_fc,
        ...
        if (snrt_is_dm_core()) {
            ...
            xdma_memcpy_nd(tcdm_in_dr, tcdm_out_dr, sstride_src_dr,
                           sstride_dst_dr, 5, tstride_src_dr, tbound_src_dr, 3,
                           tstride_dst_dr, tbound_dst_dr, 0xFFFFFFFF,
