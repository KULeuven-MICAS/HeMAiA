        ...
        if (snrt_cluster_idx() == 1) {  // Set err value for checking
            ...
            if (snrt_is_dm_core()) {
                ...
                if (interleaved_address == 1) {
                    ...
                    snrt_dma_start_2d(local_b_dma, B, 64 * sizeof(int8_t), 256,
                                      64, Cout * Kh * Kw * Cin / 64);
                }
                snrt_dma_wait_all();
                ...
                if(debug_mode){
                printf(
                    "[CONV]-DMA transfer cycle from mcycle: %d cycles for A "
                    "and B "
                    "\r\n",
            ...
            // Put the input at the starting of tcdm
            tcdm_in = (uint8_t *)(tcdm_baseaddress + delta_local_d8);
            // Put the output at the middle of tcdm
            // off-set to after the result from conv
            tcdm_out =
                (uint8_t *)(tcdm_baseaddress + delta_local_d8 + 64 * 7 * 7);
            ...
            if (snrt_is_dm_core()) {
                ...
                xdma_memcpy_nd(tcdm_in, tcdm_out, sstride_src, sstride_dst, 5,
                               tstride_src, tbound_src, 3, tstride_dst,
                               tbound_dst, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF);
                start_cycle = snrt_mcycle();
            ...
            for (int i = 1; i < 32; i++) {
                ...
                if (snrt_is_dm_core()) {
                    snrt_start_perf_counter(
                        SNRT_PERF_CNT0, SNRT_PERF_CNT_DMA_BUSY, snrt_hartid());
                    if (interleaved_address == 1) {
                        start_cycle = snrt_mcycle();
                        snrt_dma_start_1d(local_a, A,
                                          Nbatch * (H + 2 * pad_h) *
                                              ...
                                              sizeof(int8_t));
                        snrt_dma_start_1d(
                            local_b, B, Cout * Kh * Kw * Cin * sizeof(int8_t));
                    } else {
                        start_cycle = snrt_mcycle();
                        snrt_dma_start_2d(local_a_dma, A, 64 * sizeof(int8_t),
                                          256, 64,
                                          Nbatch * (H + 2 * pad_h) *
                                              (W + 2 * pad_w) * Cin / 64);
                        snrt_dma_start_2d(local_b_dma, B, 64 * sizeof(int8_t),
                                          256, 64, Cout * Kh * Kw * Cin / 64);
                    }
                    snrt_dma_wait_all();
                    end_cycle = snrt_mcycle();
                    if(debug_mode){
                        printf(
                            "[CONV-%d]-DMA transfer cycle from DMA hardware "
                            "counter %d  "
                            "for A "
                            "and B\r\n",
                            i, snrt_get_perf_counter(SNRT_PERF_CNT0));
                    }
                    snrt_reset_perf_counter(SNRT_PERF_CNT0);
                    if(debug_mode){
                        printf(
                            "[CONV-%d]-DMA transfer cycle from mcycle: %d cycles "
                            "for A "
                            "and B "
                            "\r\n",
                ...
                if (snrt_is_dm_core()) {
                    ...
                    if (interleaved_address == 1) {
                        ...
                        start_cycle = snrt_mcycle();
                        snrt_dma_start_2d(local_c_dma, C, 16 * sizeof(int32_t),
                                          256, 16 * sizeof(int32_t),
                                          M * N * meshRow * meshCol / 16);
                    }
                    snrt_dma_wait_all();
                    end_cycle = snrt_mcycle();
                ...
                if (snrt_cluster_core_idx() == 0) {
                    ...
                    if(debug_mode){
                        printf(
                            "[CONV-%d]-Compute total cycles from mcycle: %d \r\n",
                            i, end_cycle - start_cycle);
                    }
                    ...
                    if(debug_mode){
                        printf("[CONV-%d]-GeMM performance counter: %d \r\n", i,
                               performance_counter);
                    }
                };
                snrt_cluster_hw_barrier();
                ...
                // off-set to after the result from conv
                tcdm_out = (uint8_t *)(tcdm_baseaddress + delta_local_d8 +
                                       64 * 7 * 7 + i * 64);
                ...
                if (snrt_is_dm_core()) {
                    ...
                    xdma_memcpy_nd(tcdm_in, tcdm_out, sstride_src, sstride_dst,
                                   5, tstride_src, tbound_src, 3, tstride_dst,
                                   tbound_dst, 0xFFFFFFFF, 0xFFFFFFFF,
                                   0xFFFFFFFF);
                    start_cycle = snrt_mcycle();
                    ...
                    if(debug_mode){
                        printf("[MAXPOOL-%d]-XDMA cycle from mcycle: %d \r\n", i,
                               end_cycle - start_cycle);
                    }
                }
                snrt_cluster_hw_barrier();
            ...
            if (snrt_cluster_core_idx() == 0) {
                ...
                uint32_t csr0_fc = gen_csr0_config(
                    input_zp_i_fc, output_zp_i_fc, max_int_i_fc, min_int_i_fc);
                ...
                set_gemmx_csr(K_fc, N_fc, M_fc, subtraction_setting_fc, csr0_fc,
                              ...
                              shared_multiplier7_fc, M_fc * N_fc,
                              bypassSIMD_fc);
                ...
                set_gemmx_streamer_csr(
                    ...
                    D8slstride0_fc, D8slstride1_fc, D8tlbound0_fc,
                    D8tlstride0_fc, D8tlbound1_fc, D8tlstride1_fc,
                    D8tlbound2_fc, D8tlstride2_fc, set_addr_remap_index_D8_fc,
                    ...
                    D32tlbound2_fc, D32tlstride2_fc,
                    set_addr_remap_index_D32_fc,

                    delta_local_d8 + 64 * 7 * 7 + 32 * 64, delta_local_b_fc,
                    delta_local_d8_fc, delta_local_c_fc, delta_local_d32_fc,
                    bypassSIMD_fc, transposed_A_fc, transposed_B_fc,
                    channel_en_C_fc, broadcast_C_fc);
            ...
            // Put the input at the starting of tcdm
            uint8_t *tcdm_in_dr =
                (uint8_t *)tcdm_baseaddress + delta_local_d8_fc;
            // Put the output at the middle of tcdm
            uint8_t *tcdm_out_dr =
                (uint8_t *)(tcdm_baseaddress + delta_local_out_dr);
            ...
            if (snrt_is_dm_core()) {
                ...
                xdma_memcpy_nd(tcdm_in_dr, tcdm_out_dr, sstride_src_dr,
                               sstride_dst_dr, 5, tstride_src_dr, tbound_src_dr,
                               3, tstride_dst_dr, tbound_dst_dr, 0xFFFFFFFF,
