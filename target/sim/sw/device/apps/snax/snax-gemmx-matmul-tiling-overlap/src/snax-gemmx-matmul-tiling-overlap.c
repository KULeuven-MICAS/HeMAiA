...
int main() {
    if (snrt_cluster_idx() == 1) {
        ...
        // Split the TCDM to Ping-Pong Buffering
        ...
        local_d32 = (int32_t *)(snrt_l1_next() + delta_local_d32_ping);
        local_d8 = (int8_t *)(snrt_l1_next() + delta_local_d8_ping);
        ...
        // --------------------------------------------------------------------------
        // --------------------------------------------------------------------------
        // First data load, fill ping buffers before the pipeline runs
        ...
        for (int m = 0; m < M2; m++) {
            for (int n = 0; n < N2; n++) {
                for (int k = 0; k < K2; k++) {
                    ...
                    // --------------------------------------------------------------------------
                    if (snrt_is_dm_core()) {
                        next_compute_counter = current_compute_counter + 1;
                        // the index for fetching next data
                        int next_k = next_compute_counter % K2;
                        int next_n = next_compute_counter / K2 % N2;
                        int next_m = next_compute_counter / (K2 * N2) % M2;
                        ...
                        // load the correct next piece of data
                        cur_C = C + next_m * N2 * M * N * meshRow * meshCol +
                                next_n * M * N * meshRow * meshCol;
                        if (next_compute_counter % 2 == 0) {
                            snrt_dma_start_1d(
                                local_a_ping, cur_A,
                                M * K * meshRow * tileSize * sizeof(int8_t));
                            snrt_dma_start_1d(
                    ...
                    // no barrier between the compute core and the dma core
                    // snrt_cluster_hw_barrier();
                    ...
                    if (snrt_cluster_core_idx() == 0) {
                        ...
                        // Leave the last computation out of this pipeline loop
                        if (next_compute_counter == (K2 * M2 * N2)) {
                            sw_loop_break = 1;
                            break;
                        }

                        // Increment compute counter
                        current_compute_counter = next_compute_counter;
                    };
                ...
                // --------------------------------------------------------------------------
                // --------------------------------------------------------------------------
                // check the result after K times computation and accumulation
                // --------------------------------------------------------------------------
                // --------------------------------------------------------------------------
                if (snrt_cluster_core_idx() == 0) {
                    if (!bypassSIMD) {
                        ...
                        for (int i = 0; i < M * N * meshRow *
                               meshCol;
                               i++) {
                                // Check if the computed data matches the golden reference
                                // the result is always stored in the same location
                                // no double buffering
                                if (local_d8[i] !=
                                    D8_golden[i +
        ...
        if (snrt_cluster_core_idx() == 0) {
            ...
            if (current_compute_counter % 2 == 0) {
                set_gemmx_streamer_csr(
                    ...
                    delta_local_a_ping, delta_local_b_ping, delta_local_d8_ping,
                    delta_local_d32_ping, delta_local_d32_ping, bypassSIMD,
                    transposed_A, transposed_B, channel_en_C, broadcast_C);
            } else {
                set_gemmx_streamer_csr(
                    Aslstride0, Aslstride1, Atlbound0, Atlstride0, Atlbound1,
                    Atlstride1, Atlbound2, Atlstride2, Atlbound3, Atlstride3,
                    ...
                    delta_local_a_pong, delta_local_b_pong, delta_local_d8_pong,
                    delta_local_d32_pong, delta_local_d32_pong, bypassSIMD,
                    transposed_A, transposed_B, channel_en_C, broadcast_C);
            }
