	...
	if (snrt_cluster_idx() == 0) {
		...
		if (snrt_is_dm_core()) {
			...
			if (CONFIG_SIZE_CMD_0 != 0){
				local_config_data = (int32_t *)(snrt_l1_next() + delta_config_cmd_0);
				snrt_dma_start_1d(local_config_data, CONFIG_CMD,
								CONFIG_SIZE_CMD_0 * sizeof(uint32_t));
				snrt_dma_wait_all();
			}
			
			local_config_data = (int32_t *)(snrt_l1_next() + delta_config_cmd_ss);
		...
		if (snrt_is_dm_core()) {
			...
			snrt_dma_wait_all();
            printf(
                "DMA transfer cycle from DMA hardware counter %d \r\n",
		...
		if (snrt_is_compute_core()) {
			...
			// // launch_cgra_0(delta_config_data, delta_comp_data, delta_store_data, mcycle_timestamps);
			
			// // launch_cgra_0_compact(delta_config_data, delta_comp_data, delta_store_data, mcycle_timestamps);
			
			// launch_cgra_0_config(delta_config_data, delta_comp_data, delta_store_data, mcycle_timestamps);
			// launch_cgra_0_go(delta_config_data, delta_comp_data, delta_store_data, mcycle_timestamps);
