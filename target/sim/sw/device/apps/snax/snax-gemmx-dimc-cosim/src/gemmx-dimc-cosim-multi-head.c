...
int main() {
    ...
    if (snrt_cluster_idx() == 3) {
        ...
        if (snrt_is_compute_core()) {
            ...
            dimc_set_streamer_dim_w(64, 1, 64, 0, 8,
                                    (uint32_t)(tcdm_1_start_addr));
            dimc_set_streamer_dim_r0(16, 1, 256, 0, 8, (uint32_t)(buffer_ptr));
            dimc_set_streamer_dim_r1(16, 1, 256, 0, 8,
                                     (uint32_t)(buffer_ptr + 8));
            dimc_set_streamer_dim_r2(16, 1, 256, 0, 8,
                                     (uint32_t)(buffer_ptr + 16));
            dimc_set_streamer_dim_r3(16, 1, 256, 0, 8,
                                     (uint32_t)(buffer_ptr + 24));
    ...
    if (snrt_cluster_idx() == 1) {
        ...
        int8_t *cur_A, *cur_B;
        int32_t *cur_C;
        cur_A = A_blocked_row_major;
        cur_B = B_blocked_col_major;
        cur_C = C_zero;
        ...
        for (int m = 0; m < M2; m++) {
            for (int n = 0; n < N2; n++) {
                for (int k = 0; k < K2; k++) {
                    // Prepare addresses in TCDM
                    cur_A = A_blocked_row_major +
                            m * K2 * gemmx_M * gemmx_K * tileSize * meshRow +
                            k * gemmx_M * gemmx_K * tileSize * meshRow;
                    cur_B = B_blocked_col_major +
                            n * K2 * gemmx_N * gemmx_K * tileSize * meshCol +
                            k * gemmx_N * gemmx_K * tileSize * meshCol;
                    // no add c here
                    cur_C = C_zero;

                    if (snrt_is_dm_core()) {
                        snrt_dma_start_1d(tcdm_1_start_addr, cur_A,
                                          gemmx_M * gemmx_K * tileSize *
                                              meshRow * sizeof(int8_t));
                        snrt_dma_start_1d(local_b, cur_B,
                                          gemmx_N * gemmx_K * tileSize *
                                              meshCol * sizeof(int8_t));
                        snrt_dma_wait_all();
                    }
                    ...
                    if (snrt_is_compute_core()) {
                        ...
                        set_gemmx_csr(
                            gemmx_K, gemmx_N, gemmx_M, subtraction_setting,
                            csr0, csr1, shared_bitpacked_shift0,
                ...
                if (snrt_cluster_core_idx() == 0) {
                    for (int i = 0; i < (gemmx_M * gemmx_N * meshRow * meshCol);
                         i++) {
                        if (local_d32[i] !=
                ...
                if (snrt_is_dm_core()) {
                    snrt_dma_start_1d(
                        D_generated +
                            m * N2 * gemmx_M * gemmx_N * meshRow * meshCol +
                            n * gemmx_M * gemmx_N * meshRow * meshCol,
                        local_d32,
                        gemmx_M * gemmx_N * meshRow * meshCol *
                            sizeof(int32_t));
                    snrt_dma_wait_all();
                }
