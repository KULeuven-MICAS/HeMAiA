...
int main() {
    ...
    if (snrt_cluster_idx() == 1) {
        ...
        if (snrt_is_dm_core()) {
            ...
            snrt_dma_start_1d(
                local_b, B_blocked_col_major_first_half,
        ...
        if (snrt_is_compute_core()) {
            ...
            for (int i = 0; i < (gemmx_M * gemmx_N * meshRow * meshCol); i++) {
                if (local_d32[i] != D_golden_blocked_row_major_first_half[i]) {
                    err++;
                    printf(
                        "GEMMX cluster failed to verify the computed data at "
                        "index %d, expected %d but got %d\n",
                        i, D_golden_blocked_row_major_first_half[i], local_d32[i]);
                }
            }
        ...
        if (snrt_is_dm_core()) {
            snrt_dma_start_1d(
                ...
                gemmx_M * gemmx_K * tileSize * meshRow * sizeof(int8_t));
            snrt_dma_start_1d(
                local_b, B_blocked_col_major_second_half,
                gemmx_N * gemmx_K * tileSize * meshCol * sizeof(int8_t));
            snrt_dma_wait_all();
        }
        ...
        if (snrt_is_compute_core()) {
            ...
            for (int i = 0; i < (gemmx_M * gemmx_N * meshRow * meshCol); i++) {
                if (local_d32[i] != D_golden_blocked_row_major_second_half[i]) {
                    err++;
                    printf(
                        "GEMMX cluster failed to verify the computed data at "
                        "index %d, expected %d but got %d\n",
                        i, D_golden_blocked_row_major_second_half[i], local_d32[i]);
                }
            }
