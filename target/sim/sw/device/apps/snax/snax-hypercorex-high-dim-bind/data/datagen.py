...
def main():
    ...
    high_dim_bind_code_list, high_dim_bind_control_code_list = compile_hypercorex_asm(
        high_dim_bind_asm_path
    )

    # Convert each instruction to integers for input
    for i in range(len(high_dim_bind_code_list)):
        high_dim_bind_code_list[i] = hvlist2num(np.array(high_dim_bind_code_list[i]))
    ...
    data_a_list_str = format_vector_definition("uint32_t", "data_a_list", data_a_list)
    data_b_list_str = format_vector_definition("uint32_t", "data_b_list", data_b_list)
