...
asm_path = current_directory + "/dna_recog.txt"
...
# Number of classes in the dna dataset
NUM_CLASSES = 3
...
def main():
    ...
    # Loading data first
    assoc_mem_fp = hypercorex_path + "/hemaia/trained_am/hypx_dna_am.txt"
    ...
    test_samples_fp = \
        hypercorex_path + "/hemaia/test_samples/hypx_dna_test.txt"
    ...
    # Take note that each test_sample compress is 8 elements long
    # As from packing 60 4b elements into 32bits needs 7.5, append 0 leads to 8
    ...
    # Golden list of data
    golden_sub_list = [0, 1, 2] * 8
    golden_list_data = golden_sub_list * 41
    ...
    max_sub_num_predictions_str = format_scalar_definition(
        "uint32_t", "max_sub_num_predictions", max_sub_num_predictions
    )

    final_sub_num_predictions_str = format_scalar_definition(
        "uint32_t", "final_sub_num_predictions", final_sub_num_predictions
    )
    ...
    f_str = "\n\n".join(
        [
            ...
            max_sub_num_predictions_str,
            final_predictions_str,
            final_sub_num_predictions_str,
            max_num_rows_str,
            final_rows_str,
            bank_num_str,
            code_str,
