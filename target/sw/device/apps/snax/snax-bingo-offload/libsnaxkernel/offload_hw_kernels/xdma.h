...
// The THIRD monoid of the collective algebra: distributed MoE top-k expert routing (StreamTopKMergeRt).
// Same push mechanics + accEn semantics as moment_merge/normstat -- only the writer extension differs. A beat
// carries (logit, expert-id) candidate pairs: value_k = lane k (fp32), index_k = lane (8+k) (uint32); the
// writer sorts+merges them in-fabric into the running global top-8 table. GENUINELY beyond-SHARP: a
// compare-exchange sort, not a linear reduce. In accEn mode each shard pushes its LOCAL top-8 candidate list
// and the slot accumulates the global top-8; the host reads the top-k prefix (k<=8, covers all real MoE).
static inline uint32_t xdma_topk_push_run(uint64_t src_addr, uint64_t dst_addr, uint32_t nvalid,
                                          uint32_t acc_en, uint32_t acc_init, uint32_t acc_slot)
{
    BINGO_TRACE_MARKER(BINGO_TRACE_XDMA_CFG_START);
#ifdef WRITER_EXT_STREAMTOPKMERGERT
    xdma_disable_all_extensions();
    uint32_t csr[1] = { (nvalid & 0xFFu)
                        | ((acc_en   & 0x1u) << 8)
                        | ((acc_init & 0x1u) << 9)
                        | ((acc_slot & 0x7u) << 10) };
    xdma_enable_dst_ext(WRITER_EXT_STREAMTOPKMERGERT, csr);
    BINGO_XDMA_TRY(xdma_memcpy_1d_full_addr(src_addr, dst_addr, XDMA_WIDTH), "xdma_topk_push");
    BINGO_TRACE_MARKER(BINGO_TRACE_XDMA_CFG_END);
    BINGO_TRACE_MARKER(BINGO_TRACE_XDMA_RUN_START);
    xdma_task_t task_id = xdma_start();
    xdma_wait_task(task_id);
    BINGO_TRACE_MARKER(BINGO_TRACE_XDMA_RUN_END);
    xdma_disable_dst_ext(WRITER_EXT_STREAMTOPKMERGERT);
    return BINGO_RET_SUCC;
#else
    printf_safe("[Cluster %d Core %d]: Error! xdma_topk_push needs WRITER_EXT_STREAMTOPKMERGERT "
                "(cfg/snax_xdma_test.hjson) -- this cluster's writer_extensions doesn't have it.\r\n",
                snrt_cluster_idx(), snrt_cluster_core_idx());
    (void)src_addr; (void)dst_addr; (void)nvalid;
    return BINGO_RET_FAIL;
...
// The top-k routing monoid entry -- same arg layout as moment_merge_push (reuses its args struct: identical
// fields src/dst/nvalid/accEn/accInit/accSlot).
SNAX_LIB_DEFINE uint32_t __snax_bingo_kernel_xdma_topk_push(void *arg)
{
    BINGO_SW_GUARD_CHECK(arg, __snax_bingo_kernel_xdma_moment_merge_push_args_t);
    if (snrt_is_dm_core()) {
        BINGO_TRACE_MARKER(BINGO_TRACE_KERNEL_ARG_PARSE_START);
        ...
        bingo_kernel_scratchpad_t* sp = BINGO_GET_SP(arg, __snax_bingo_kernel_xdma_moment_merge_push_args_t);
        BINGO_TRACE_MARKER(BINGO_TRACE_KERNEL_ARG_PARSE_END);
        if (xdma_topk_push_run(src_addr, dst_addr, nvalid, acc_en, acc_init, acc_slot) != BINGO_RET_SUCC)
            return BINGO_RET_FAIL;
        sp->return_value = (uint32_t)dst_addr;
        sp->num_return_values = 0;
        return BINGO_RET_SUCC;
