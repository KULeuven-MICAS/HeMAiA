...
def emit_matmul_data(**kwargs):
    ...
    # Generate test data. A_mem/B_mem/C_mem retain the exact byte
    # representation consumed by the streamer; they are written to
    # mempool.bin instead of being compiled into data.h.
    if (
        snax_acc_cfg["snax_versacore_input_a_data_type"][data_type] == "Float"
    ):  # FP8 data type
        ...
        A_mem = np.asarray(A_fp8_bin, dtype=np.uint8)
        B_mem = np.asarray(B_fp8_bin, dtype=np.uint8)
        A_golden = A_fp8.copy()
        B_golden = B_fp8.copy()
        ...
        C_mem = np.asarray(float32_to_hex_uint(C), dtype=np.uint32)
        ...
        if a_len == 4:
            ...
            A_mem = np.asarray(A_packed, dtype=np.int8)
        elif a_len == 8:
            A_mem = np.asarray(A, dtype=np.int8)
        elif a_len == 16:
            A_mem = np.asarray(A, dtype=np.int16)
        else:
        ...
        if b_len == 4:
            ...
            B_mem = np.asarray(B_packed, dtype=np.int8)
        elif b_len == 8:
            B_mem = np.asarray(B, dtype=np.int8)
        else:
        ...
        C_mem = np.asarray(C, dtype=np.int32)

    if kwargs["transposed_A"] == 1:
        A_golden = A_golden.reshape(M, K, meshRow, tileSize)
        A_golden = A_golden.transpose(0, 1, 3, 2).reshape(-1)
    if kwargs["transposed_B"] == 1:
        B_golden = B_golden.reshape(K, N, tileSize, meshCol)
        B_golden = B_golden.transpose(0, 1, 3, 2).reshape(-1)
    ...
    # -----------------------------------------------------------
    # Region starts are cache-line aligned so every memory-chip -> TCDM DMA has
    # a naturally aligned source. D remains in TCDM and is checked against the
    # compact golden arrays emitted above.
    def _align64(value):
        return (int(value) + 63) // 64 * 64

    a_image = A_mem.tobytes()
    b_image = B_mem.tobytes()
    c_image = C_mem.tobytes()
    assert len(a_image) == int(a_data_length), (len(a_image), int(a_data_length))
    assert len(b_image) == int(b_data_length), (len(b_image), int(b_data_length))
    assert len(c_image) == int(c_data_length), (len(c_image), int(c_data_length))

    mem_chip_local_base = int(kwargs.get("mem_chip_local_base", 0x80000000))
    mem_off_A = 0
    mem_off_B = _align64(mem_off_A + len(a_image))
    mem_off_C = _align64(mem_off_B + len(b_image))
    image_bytes = mem_off_C + len(c_image)

    data_str += [
        format_scalar_definition("uint64_t", "mem_chip_local_base", mem_chip_local_base)
    ]
    data_str += [format_scalar_definition("uint32_t", "mem_off_A", mem_off_A)]
    data_str += [format_scalar_definition("uint32_t", "mem_off_B", mem_off_B)]
    data_str += [format_scalar_definition("uint32_t", "mem_off_C", mem_off_C)]

    image = bytearray(image_bytes)
    image[mem_off_A : mem_off_A + len(a_image)] = a_image
    image[mem_off_B : mem_off_B + len(b_image)] = b_image
    image[mem_off_C : mem_off_C + len(c_image)] = c_image
