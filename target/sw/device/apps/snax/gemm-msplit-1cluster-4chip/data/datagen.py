...
def emit_matmul_data(**kwargs):
    ...
    # M-split across chips: B is shared by all chips, while A and D are split
    # along M into `num_chips` row-blocks (one block per compute chip). The
    # streamer / TCDM footprint below is sized to a SINGLE block (M_block),
    # whereas the A/B data and the golden D further down stay sized to the full
    # M, because the memory-chip image holds the assembled full matrices.
    num_chips = int(kwargs["num_chips"])
    assert M % num_chips == 0, "M must be divisible by num_chips"
    M_full = M
    M_block = M // num_chips

    data_str += [format_scalar_definition("uint32_t", "M", M_full)]
    data_str += [format_scalar_definition("uint32_t", "M_block", M_block)]
    data_str += [format_scalar_definition("uint32_t", "num_chips", num_chips)]
    data_str += [format_scalar_definition("uint32_t", "K", K)]
    data_str += [format_scalar_definition("uint32_t", "N", N)]

    # From here on, the per-chip streamer/TCDM math uses one block worth of M.
    M = M_block
    ...
    if (
        ...
        if enable_full_C == 1:
            C = np.random.randint(
                C_MIN, C_MAX, size=(M_full, N, meshRow, meshCol)
            ).reshape(-1)
        else:
    ...
    # transposed_A/B on load -- exactly like the single-cluster reference, which
    # DMAs the pre-transpose A[]/B[] arrays. The golden D below uses the
    # post-transpose A/B, matching the HW result.
    A_mem = np.asarray(A, dtype=np.int8).copy()
    B_mem = np.asarray(B, dtype=np.int8).copy()

    if kwargs["transposed_A"] == 1:
        A = A.reshape(M_full, K, meshRow, tileSize)
    ...
    # -----------------------------------------------------------
    # The memory chip holds, in order: the shared B matrix, then the num_chips
    # A row-blocks, then a zeroed D region (one block per chip; the device
    # writes its computed D-block back here). Region starts are 64-byte aligned.
    def _align64(value):
        return (int(value) + 63) // 64 * 64

    mem_chip_local_base = int(kwargs.get("mem_chip_local_base", 0x80000000))
    a_block_bytes = int(M_block * K * meshRow * tileSize * a_len // 8)
    b_total_bytes = int(K * N * tileSize * meshCol * b_len // 8)
    d_block_bytes = int(M_block * N * meshRow * meshCol * c_len // 8)

    mem_off_B = 0
    mem_off_A_base = _align64(mem_off_B + b_total_bytes)
    mem_off_D_base = _align64(mem_off_A_base + num_chips * a_block_bytes)
    image_bytes = mem_off_D_base + num_chips * d_block_bytes
    ...
    assert len(a_image) == num_chips * a_block_bytes, (
        len(a_image),
        num_chips * a_block_bytes,
    )
...
def main():
    ...
    merged_config = {**param, **hw, "out_dir": args.out_dir}
