...
def emit_matmul_data(**kwargs):
    ...
    # M-split across clusters: B is shared by all clusters, while A and D are
    # split along M into `num_clusters` row-blocks (one block per cluster). The
    ...
    num_clusters = int(kwargs.get("num_clusters", kwargs.get("num_chips", 4)))
    assert M % num_clusters == 0, "M must be divisible by num_clusters"
    M_full = M
    M_block = M // num_clusters

    data_str += [format_scalar_definition("uint32_t", "M", M_full)]
    data_str += [format_scalar_definition("uint32_t", "M_block", M_block)]
    data_str += [format_scalar_definition("uint32_t", "num_clusters", num_clusters)]
    data_str += [format_scalar_definition("uint32_t", "num_chips", num_clusters)]
    data_str += [format_scalar_definition("uint32_t", "K", K)]
    data_str += [format_scalar_definition("uint32_t", "N", N)]

    # From here on, the per-cluster streamer/TCDM math uses one block worth of M.
    M = M_block
