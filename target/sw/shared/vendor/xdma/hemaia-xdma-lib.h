...
#define HEMAIA_XDMA_CFG_ADDR 0xFFFFB000ULL
static inline uint32_t hemaia_read_xdma_cfg_reg(uint32_t addr) {
    volatile uint32_t* base =
        (volatile uint32_t*)chiplet_addr_transform(HEMAIA_XDMA_CFG_ADDR);
    return base[addr];
}
...
int32_t hemaia_xdma_multicast_1d(const void* src, void** dst, uint32_t dst_num,
                                 uint32_t size) {
    if (size % XDMA_WIDTH != 0) {
        XDMA_DEBUG_PRINT("Size is not multiple of XDMA_WIDTH\n");
        return -1;
...
// Only for move data inside the same memory
