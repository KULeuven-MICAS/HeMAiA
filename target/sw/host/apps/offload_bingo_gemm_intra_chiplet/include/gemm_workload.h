...
uint32_t __workload_versacore(bingo_task_t **task_list, uintptr_t *output_data_ptr){
    ...
    uintptr_t output_data_addr = (uintptr_t)o1heapAllocate(local_l3_heap_manager, ARRAY_SIZE_BYTES(D));
    ...
    // 2. Register the tasks
    bingo_task_t *task_versacore = bingo_task_create(__snax_kernel_gemm_intra_chiplet_func_addr, (uint32_t)(uintptr_t)(&gemm_args), current_chip_id, cluster_id);
