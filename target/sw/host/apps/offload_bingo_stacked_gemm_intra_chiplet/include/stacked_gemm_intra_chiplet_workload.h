...
uint32_t __workload_versacore_stacked_gemm_intra_chiplet(bingo_task_t** task_list) {
    ...
    uint32_t A1dataTileSize = BINGO_CHIPLET_READW(M1) * BINGO_CHIPLET_READW(K1) *
    ...
    uint32_t D1dataSize = BINGO_CHIPLET_READW(M1) * BINGO_CHIPLET_READW(N1) *
                         ...
                         uint32_t B2dataSize = BINGO_CHIPLET_READW(N1) * BINGO_CHIPLET_READW(N2) *
                         BINGO_CHIPLET_READW(meshCol) *
                         BINGO_CHIPLET_READW(tileSize) * sizeof(uint8_t);
                         uint32_t D2datasize = BINGO_CHIPLET_READW(M1) * BINGO_CHIPLET_READW(N2) *
                         BINGO_CHIPLET_READW(meshRow) *
                         BINGO_CHIPLET_READW(meshCol) * sizeof(uint32_t);
    ...
    uint64_t cluster_l1_addr_A1 = bingo_l1_alloc(current_chip_id, 0, BINGO_CHIPLET_READW(A1dataTileSize));
    ...
    gemm1_args->array_shape
        = BINGO_CHIPLET_READW(array_shape);
    ...
    gemm2_args->M = BINGO_CHIPLET_READW(M1);
    gemm2_args->K = BINGO_CHIPLET_READW(N1);
    gemm2_args->N = BINGO_CHIPLET_READW(N2);
    gemm2_args->array_shape
        = BINGO_CHIPLET_READW(array_shape);
