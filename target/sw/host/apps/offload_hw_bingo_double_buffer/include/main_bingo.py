...
def create_dfg(params, mem_handles):
    ...
    for i in range(params['num_double_buffers']):
        ...
        node_copy_A = BingoNode(
            assigned_chiplet_id=0,
            assigned_cluster_id=0,
            assigned_core_id=1, # Core 1 for Load
            kernel_name="__snax_bingo_kernel_idma_1d_copy",
            kernel_args=SnaxBingoKernelIdma1dCopyArgs(
        ...
        if i == 0:
            node_gemm = BingoNode(
                assigned_chiplet_id=0,
                assigned_cluster_id=0,
                assigned_core_id=0, # Core 0 for Compute
        ...
        node_copy_D = BingoNode(
            assigned_chiplet_id=0,
            assigned_cluster_id=0,
            assigned_core_id=2, # Core 2 for Store
...
def compile_dfg(bingo_dfg, output_dir):
    """Compiles the DFG: performs transformations and emits C code."""
    
    # 1. Transformations
    # Add Entry Node
    bingo_dfg.bingo_transform_dfg_add_entry_node()
    # Add Exit Nodes
    bingo_dfg.bingo_transform_dfg_add_exit_nodes()
    bingo_dfg.bingo_visualize_dfg(
    ...
    bingo_dfg.bingo_visualize_dfg(os.path.join(output_dir, "final_dfg"))
    bingo_dfg.bingo_export_dfg_to_csv(
    ...
    # 2. Emission
    # Emit offload_hw_bingo.h
    bingo_dfg.bingo_emit_offload_c_code(
        output_path=os.path.join(output_dir, "offload_hw_bingo.h"),
