...
#
# E-d IN-FABRIC arm -- KV-sharded decode softmax epilogue with the cross-shard combine
# done by the in-fabric moment-merge collective.
#
# P clusters each own a KV shard of S_p scores. Each cluster computes its OWN partial
# softmax statistics on the SIMD datapath, then the P partials are folded across clusters
# by StreamMomentMergeRt as they cross the fabric. No core ever runs the cross-shard
# merge -- that is the offload win this workload measures against the gather-then-local
# baseline (ed_decode_p2_baseline).
#
...
def get_args():
    p = argparse.ArgumentParser(description="ed_decode_p2_infabric")
...
def main():
    ...
    merge_push = BingoNode(
        ...
        node_name="MergePush", kernel_name="__snax_bingo_kernel_xdma_moment_merge_push",
        kernel_args=SnaxBingoKernelXdmaMomentMergePushArgs(src_addr=stage, dst_addr=dest, nvalid=P))
    ...
    for p in range(P):
        ...
        nodes += [push_m, push_l]
        edges += [(a2_node, push_m), (d_node, push_l), (push_m, merge_push), (push_l, merge_push)]
    ...
    check = BingoNode(
        ...
        kernel_args=HostBingoKernelCheckResultArgs(
            ...
            num_elements=2, name="ed_decode_p2_infabric"))
    nodes += [store, check]
    ...
    dfg.bingo_compile_dfg(
        "E-d KV-sharded decode softmax epilogue (in-fabric)", args.output_dir,
        args.output_offload_file_name, extra_include_header_list=["ed_decode_data.h"])
    print("Generated ed_decode_p2_infabric DFG")
