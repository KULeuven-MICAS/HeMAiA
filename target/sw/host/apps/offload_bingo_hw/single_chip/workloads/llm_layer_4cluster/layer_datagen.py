...
"""Operands and goldens for one toy transformer layer.
...
def generate_layer_data(p):
    ...
    for nm in ("q", "k", "v"):
        ...
        y = _to_fp16(n1_q.astype(np.int32) @ w.astype(np.int32))
        proj[nm] = y
    ...
    out["fa_k_a"] = row_major_to_a(ki, T // mr, d // ts, mr, ts)
    ...
    ctx_ = (attn @ vi.astype(np.float32)).astype(np.float16)
    ...
    attn_out = _to_fp16(ctx_q.astype(np.int32) @ w_o.astype(np.int32))
...
def stage(st, data, p):
    ...
    for nm in ("norm1_golden", "rope_q_golden", "attn_golden", "proj_o_golden",
               "resid1_golden", "norm2_golden", "ffn_golden", "layer_golden"):
