...
def emit_xdma_data(**kwargs):
    ...
    # 10. ELEMENTWISE-ADD (int32 writer extension): dst = sum of N operands.
    # Each operand has eltadd_int32_per_operand int32 (multiple of 16 for the
    # 512-bit bus); operands are laid out contiguously in eltadd_input.
    eltadd_num_operands = 4
    eltadd_int32_per_operand = 64
    ea = np.random.randint(-100000, 100000,
                           size=(eltadd_num_operands, eltadd_int32_per_operand),
                           dtype=np.int32)
    golden_eltadd = ea.sum(axis=0, dtype=np.int32)
    golden_eltadd_binary = (ea[0] + ea[1]).astype(np.int32)
    data_str += [format_scalar_definition("uint32_t", "eltadd_num_operands", eltadd_num_operands)]
    data_str += [format_scalar_definition("uint32_t", "eltadd_int32_per_operand", eltadd_int32_per_operand)]
    data_str += [format_vector_definition("int32_t", "eltadd_input", ea.reshape(-1))]
    data_str += [format_vector_definition("int32_t", "golden_eltadd", golden_eltadd)]
    data_str += [format_vector_definition("int32_t", "golden_eltadd_binary", golden_eltadd_binary)]
