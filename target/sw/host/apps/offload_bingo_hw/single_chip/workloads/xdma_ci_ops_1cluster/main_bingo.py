...
def layout_conversions(M_T, K_T, N_T, meshRow, tileSize, meshCol, elem_bytes):
    ...
    rm2b, b2rm = (xdma_conv_args("xdma_row_major_to_b"),
                  xdma_conv_args("xdma_b_to_row_major"))
    rm2d, d2rm = (xdma_conv_args("xdma_row_major_to_d"),
                  xdma_conv_args("xdma_d_to_row_major"))
    return [
...
def main():
    ...
    # ── 10. ELEMENTWISE-ADD (int32 writer extension) ─────────────
    # General: dst = sum of N operand buffers (operands contiguous in L1).
    # Binary:  dst = operand0 + operand1 (separate L1 buffers; the kernel
    #          derives the operand stride from the two addresses).
    ea_n_op = 4
    ea_per_op = 64                       # int32 per operand (multiple of 16)
    ea_in_bytes = ea_n_op * ea_per_op * 4
    ea_out_bytes = ea_per_op * 4
    ea_operand_stride = ea_per_op * 4

    l1_ea_in = BingoMemAlloc("l1_ea_in", size=ea_in_bytes, mem_level="L1", chip_id=0, cluster_id=0)
    load_ea = BingoNode(
        assigned_chiplet_id=0, assigned_cluster_id=0, assigned_core_id=DMA_CORE,
        node_name="Load_eltadd",
        kernel_name="__snax_bingo_kernel_idma_1d_copy",
        kernel_args=SnaxBingoKernelIdma1dCopyArgs(BingoMemSymbol("eltadd_input"), l1_ea_in, ea_in_bytes))
    dfg.bingo_add_node(load_ea)
    dfg.bingo_add_edge(prev_chk, load_ea)
    ...
    xdma_eltadd = BingoNode(
        ...
        kernel_args=SnaxBingoKernelXdmaElementwiseAddArgs(
            l1_ea_in, l1_ea_out, ea_per_op, ea_n_op, ea_operand_stride))
    dfg.bingo_add_node(xdma_eltadd)
    dfg.bingo_add_edge(load_ea, xdma_eltadd)
    prev_chk = add_xdma_test(dfg, "eltadd", l1_ea_out, xdma_eltadd,
                             BingoMemSymbol("golden_eltadd"), ea_out_bytes, HOST_CORE)

    # Binary: load operand0 -> l1_ea_a and operand1 -> l1_ea_b in two buffers.
    # The kernel derives operand_stride = src_b - src_a; the xDMA reader can
    # only stride FORWARD, so src_a must sit at a lower address than src_b.
    # The L1 bump allocator assigns addresses in name-sorted order, so naming
    # operand0's buffer "l1_ea_a" (< "l1_ea_b") guarantees a positive stride.
    l1_ea_a = BingoMemAlloc("l1_ea_a", size=ea_out_bytes, mem_level="L1", chip_id=0, cluster_id=0)
    l1_ea_b = BingoMemAlloc("l1_ea_b", size=ea_out_bytes, mem_level="L1", chip_id=0, cluster_id=0)
    load_ea_a = BingoNode(
        assigned_chiplet_id=0, assigned_cluster_id=0, assigned_core_id=DMA_CORE,
        node_name="Load_eltadd_a",
        kernel_name="__snax_bingo_kernel_idma_1d_copy",
        kernel_args=SnaxBingoKernelIdma1dCopyArgs(
            BingoMemSymbol("eltadd_input"), l1_ea_a, ea_out_bytes))
    load_ea_b = BingoNode(
        assigned_chiplet_id=0, assigned_cluster_id=0, assigned_core_id=DMA_CORE,
        node_name="Load_eltadd_b",
        kernel_name="__snax_bingo_kernel_idma_1d_copy",
        kernel_args=SnaxBingoKernelIdma1dCopyArgs(
            BingoMemSymbol("eltadd_input", offset=ea_operand_stride), l1_ea_b, ea_out_bytes))
    dfg.bingo_add_node(load_ea_a)
    dfg.bingo_add_node(load_ea_b)
    dfg.bingo_add_edge(prev_chk, load_ea_a)
    dfg.bingo_add_edge(load_ea_a, load_ea_b)
    ...
    xdma_eltadd_bin = BingoNode(
        ...
        kernel_args=SnaxBingoKernelXdmaElementwiseAddAbArgs(
            l1_ea_a, l1_ea_b, l1_ea_bin_out, ea_per_op))
    dfg.bingo_add_node(xdma_eltadd_bin)
    dfg.bingo_add_edge(load_ea_b, xdma_eltadd_bin)
    prev_chk = add_xdma_test(dfg, "eltadd_binary", l1_ea_bin_out, xdma_eltadd_bin,
                             BingoMemSymbol("golden_eltadd_binary"), ea_out_bytes, HOST_CORE)
