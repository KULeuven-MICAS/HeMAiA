...
#
# Per-op xDMA workload: functional check + cycle sweep for the "a_to_row" op.
# Each CONFIGS entry runs Load -> xDMA a_to_row -> Store -> Check and emits one
...
# Per-op VersaCore A-layout to row-major conversion sweep over nine
# configurations.
#
# Task dependency graph:
#
# For each config i = 0..8:
#   Load_i -> Op_i -> Store_a_to_row_cfg[i] -> Check_a_to_row_cfg[i]
#
# Config ordering:
#   Check_a_to_row_cfg[i] -> Load_[i+1]    for i = 0..7
...
if __name__ == "__main__":
    run_op_workload("a_to_row", CONFIGS)
