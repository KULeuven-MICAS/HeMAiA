...
def emit_header_file(**kwargs):
    num_data = int(kwargs.get("num_data", 8))
    data_bytes = int(kwargs.get("data_bytes", 1024))
    if num_data != 8:
        raise ValueError(
            f"xdma_read_from_chip00_4chiplet_2cluster expects num_data=8, got {num_data}"
        )
    if data_bytes <= 0:
        raise ValueError(f"data_bytes must be positive, got {data_bytes}")

    total_bytes = num_data * data_bytes
    local_l3_bytes = 2 * data_bytes
    recv_bytes = (num_data - 2) * data_bytes
    ...
    lines = [
        ...
        "// Per-chip local golden image for A1..A8. The memory-chip binary uses the same bytes.",
        format_vector_definition(
            "uint8_t",
            "A_golden_l3",
            golden,
        ...
        "// Per-chiplet L3 staging for the two local cluster chunks.",
        format_vector_declaration(
            "uint8_t",
            "A_local_l3",
            [0] * local_l3_bytes,
        ...
        "// Chip00 receive slots for A3..A8 copied back from chiplets 01, 10, 11.",
        format_vector_declaration(
            "uint8_t",
            "A_recv_chip00_l3",
            [0] * recv_bytes,
