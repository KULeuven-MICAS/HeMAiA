...
int main() {
    ...
    volatile comm_buffer_t* comm_buffer_ptr =
        o1heapAllocate(local_l3_heap_manager, sizeof(comm_buffer_t));
    if (comm_buffer_ptr == NULL) {
        printf(
            "Chip(%x, %x): [Host] Error when allocating comm buffer, l3 heap "
            "manager = 0x%lx, size = %x\r\n",
            get_current_chip_loc_x(), get_current_chip_loc_y(),
            local_l3_heap_manager, sizeof(comm_buffer_t));
        return -1;
    }
