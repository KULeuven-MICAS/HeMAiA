...
int main() {
    ...
    for (int si = 0; si < TIMING_NUM_SIZES; si++) {
        ...
        for (int rep = 0; rep < TIMING_NUM_REPS; rep++) {
            ...
            // Dequantize (int32 in, fp32 out, scale, n, scratchpad).
            // Fill the int32 source OUTSIDE the timing window.
            for (uint64_t i = 0; i < N; i++)
                timing_int32_src[i] = (int32_t)(op_a_big[i] * 100.0f);
...

            {
                ...
                printf("CHECK,dequantize,%lu,%s\r\n", N, errs ? "FAIL" : "PASS");
            }
        }
    }

    printf("=== ara dequantize done ===\r\n");
    return 0;
}
