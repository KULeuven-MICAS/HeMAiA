...
// kernel "reciprocal" (bingo dispatches it to CVA6+Ara).  Per size it prints:
//   CYCLES,reciprocal,<N>,<rep>,<cycles>   (timing)
//   CHECK,reciprocal,<N>,PASS|FAIL         (output vs golden from util/sim/ara_lib.py)
...
int main() {
    ...
    for (int si = 0; si < TIMING_NUM_SIZES; si++) {
        ...
        for (int rep = 0; rep < TIMING_NUM_REPS; rep++) {
            ...
            __host_bingo_kernel_fp32_reciprocal(t_args);
            c1 = ara_get_cycle_count();
            printf("CYCLES,reciprocal,%lu,%d,%lu\r\n", N, rep, c1 - c0);

            {
                ...
                printf("CHECK,reciprocal,%lu,%s\r\n", N, errs ? "FAIL" : "PASS");
            }
        }
    }

    printf("=== ara reciprocal done ===\r\n");
    return 0;
}
