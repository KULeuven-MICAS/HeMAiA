...
// kernel "neg" (bingo dispatches it to CVA6+Ara).  Per size it prints:
//   CYCLES,neg,<N>,<rep>,<cycles>   (timing)
//   CHECK,neg,<N>,PASS|FAIL         (output vs golden from util/sim/ara_lib.py)
...
int main() {
    ...
    for (int si = 0; si < TIMING_NUM_SIZES; si++) {
        ...
        for (int rep = 0; rep < TIMING_NUM_REPS; rep++) {
            ...
            __host_bingo_kernel_fp32_neg(t_args);
            c1 = ara_get_cycle_count();
            printf("CYCLES,neg,%lu,%d,%lu\r\n", N, rep, c1 - c0);
