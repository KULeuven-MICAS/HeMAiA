...
// kernel "silu_mul" (bingo dispatches it to CVA6+Ara).  Per size it prints:
//   CYCLES,silu_mul,<N>,<rep>,<cycles>   (timing)
//   CHECK,silu_mul,<N>,PASS|FAIL         (output vs golden from util/sim/ara_lib.py)
...
int main() {
    ...
    for (int si = 0; si < TIMING_NUM_SIZES; si++) {
        ...
        for (int rep = 0; rep < TIMING_NUM_REPS; rep++) {
            ...
            __host_bingo_kernel_fp32_silu_mul(t_args);
            c1 = ara_get_cycle_count();
            printf("CYCLES,silu_mul,%lu,%d,%lu\r\n", N, rep, c1 - c0);

            {
                ...
                printf("CHECK,silu_mul,%lu,%s\r\n", N, errs ? "FAIL" : "PASS");
            }
        }
    }

    printf("=== ara silu_mul done ===\r\n");
    return 0;
}
