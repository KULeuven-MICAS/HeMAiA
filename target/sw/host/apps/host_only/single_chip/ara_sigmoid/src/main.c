...
// kernel "sigmoid" (bingo dispatches it to CVA6+Ara).  Per size it prints:
//   CYCLES,sigmoid,<N>,<rep>,<cycles>   (timing)
//   CHECK,sigmoid,<N>,PASS|FAIL         (output vs golden from util/sim/ara_lib.py)
...
int main() {
    ...
    for (int si = 0; si < TIMING_NUM_SIZES; si++) {
        ...
        for (int rep = 0; rep < TIMING_NUM_REPS; rep++) {
            ...
            __host_bingo_kernel_fp32_sigmoid(t_args);
            c1 = ara_get_cycle_count();
            printf("CYCLES,sigmoid,%lu,%d,%lu\r\n", N, rep, c1 - c0);

            {
                ...
                printf("CHECK,sigmoid,%lu,%s\r\n", N, errs ? "FAIL" : "PASS");
            }
        }
    }

    printf("=== ara sigmoid done ===\r\n");
    return 0;
}
