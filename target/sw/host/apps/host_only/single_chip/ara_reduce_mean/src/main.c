...
// kernel "reduce_mean" (bingo dispatches it to CVA6+Ara).  Per size it prints:
//   CYCLES,reduce_mean,<N>,<rep>,<cycles>   (timing)
//   CHECK,reduce_mean,<N>,PASS|FAIL         (output vs golden from util/sim/ara_lib.py)
...
int main() {
    ...
    for (int si = 0; si < TIMING_NUM_SIZES; si++) {
        ...
        for (int rep = 0; rep < TIMING_NUM_REPS; rep++) {
            ...
            // Reduction (input, scalar_out, n).
            t_args[0] = (uint64_t)(uintptr_t)op_a_big;
            t_args[1] = (uint64_t)(uintptr_t)&t_scalar_out;
            t_args[2] = N;
            c0 = ara_get_cycle_count();
            __host_bingo_kernel_fp32_reduce_mean(t_args);
            c1 = ara_get_cycle_count();
            printf("CYCLES,reduce_mean,%lu,%d,%lu\r\n", N, rep, c1 - c0);

            {
                float g = golden_reduce[si];
                float d = t_scalar_out - g; if (d < 0) d = -d;
                float s = g < 0 ? -g : g; if (s < 1.0f) s = 1.0f;
                printf("CHECK,reduce_mean,%lu,%s\r\n", N, (d / s > ARA_TOL) ? "FAIL" : "PASS");
            }
        }
    }

    printf("=== ara reduce_mean done ===\r\n");
    return 0;
}
