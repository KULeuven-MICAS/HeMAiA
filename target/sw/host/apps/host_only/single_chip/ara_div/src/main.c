...
// kernel "div" (bingo dispatches it to CVA6+Ara).  Per size it prints:
//   CYCLES,div,<N>,<rep>,<cycles>   (timing)
//   CHECK,div,<N>,PASS|FAIL         (output vs golden from util/sim/ara_lib.py)
...
int main() {
    ...
    for (int si = 0; si < TIMING_NUM_SIZES; si++) {
        ...
        for (int rep = 0; rep < TIMING_NUM_REPS; rep++) {
            ...
            __host_bingo_kernel_fp32_div(t_args);
            c1 = ara_get_cycle_count();
            printf("CYCLES,div,%lu,%d,%lu\r\n", N, rep, c1 - c0);
