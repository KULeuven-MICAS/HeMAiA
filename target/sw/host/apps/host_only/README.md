...
- **ara_\<kernel\>** (ara_add, ara_exp, ara_softmax, ...): Per-kernel cycle-count characterization for the FP32 RVV host kernels. One app per kernel, each self-contained (its own `src/main.c`), timing a single kernel and generating its test data via the shared `util/sim/ara/ara_lib.py`. Driven as a sweep by `target/sim/automation/sweep/ara/`.
