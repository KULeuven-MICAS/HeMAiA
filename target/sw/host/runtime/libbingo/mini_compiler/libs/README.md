...
| [`block/`](block/) | The blocks: attention, its fold, the MoE FFN. |
| [`verify/`](verify/) | Host-side readbacks and comparisons, named by precision. |

## Every port carries four things

Shape, **layout**, **precision** and **location** — and all four are part of the signature
...
- a **location** in a memory pool the platform does not have is simply unmapped. The loads
  return whatever the fabric gives back and the checks compare garbage against garbage.
...
own layout or location gaps must declare `closes_gaps` — the conversion happens in the
*block's* build, in the block's own order.
...
from libs.block import Attention, fa_gather

blk = Attention(bc=32, br=32, dhead=128, nkv=8, clusters=4, decomp="kvsplit")
