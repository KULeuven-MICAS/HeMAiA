...
# param_path = "/users/micas/fkong/Documents/proj_XDMA/HeMAiA/target/sim/sw/device/apps/snax/snax-xdma-kvcache-load/data/params.hjson"
# binary_path = "/users/micas/fkong/Documents/proj_XDMA/HeMAiA/target/sim_chip/bin/occamy_chip.vlt"
# hemaia_root_path = "/users/micas/fkong/Documents/proj_XDMA/HeMAiA/"

# The function needs to be parallelized
def process_configuration(folder_name, binary_path, result_csv_path, M, N, input_layout, output_layout):
    ...
    with open(os.path.join(folder_name, "uart_0_0.log"), "r") as log_file:
        baseline_cycle_line = next((ln for ln in log_file if "The IDMA load is finished in" in ln), "")
        baseline_cycle_val = baseline_cycle_line.split("in")[-1].split("cycles")[0].strip() if baseline_cycle_line else "N/A"

        xdma_cycle_line = next((ln for ln in log_file if "The XDMA load is finished in" in ln), "")
    ...
    with open(result_csv_path, "a") as result_file:
        result_file.write(f"{M},{N},{input_layout},{output_layout},{baseline_cycle_val},{xdma_cycle_val}\n")
...
#                     folder_name = f"M_{M}_N_{N}_IL_{input_layout}_OL_{output_layout}"
...
#                     futures.append(executor.submit(process_configuration, folder_name, M, N, input_layout, output_layout))
...
def main():
    parser = argparse.ArgumentParser(description='Automation test on xmda kvcache load')

    parser.add_argument(
        "--param_path",
        required=True,
        help="test param path"
    )

    parser.add_argument(
        "--binary_path",
        required=True,
        help="verilator bin path"
    )

    parser.add_argument(
        "--hemaia_root_path",
        required=True,
        help="hemaia root path"
    )
    parser.add_argument(
        "--num_threads",
        type=int,
    ...
    hemaia_root_path = args.hemaia_root_path
    num_threads = args.num_threads
    Ms = range(1024, 2048, 32)
    Ns = [512]
    input_layouts = ["MN"]
    ...
    with open(result_csv_path, 'w') as result_csv:
        ...
        result_csv.write("M,N,input_layout,output_layout,idma,xdma\n")
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        futures = []
        for M in Ms:
            for N in Ns:
                for input_layout in input_layouts:
                    for output_layout in output_layouts:
                        ...
                        # 4. Copy the "snax-xdma-kvcache-load" folder to the new folder
                        source_dir = os.path.join(apps_path, "offload-snax-xdma-kvcache-load")
                        target_dir = os.path.join(folder_name, "app_chip_0_0")
                        shutil.copytree(source_dir, target_dir)

                        futures.append(executor.submit(process_configuration, folder_name, binary_path, result_csv_path, M, N, input_layout, output_layout))
