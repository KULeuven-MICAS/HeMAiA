...
# modify the occamy_chip.vsim to

# #!/bin/bash
# script_dir="$( cd -- "$( dirname -- "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
# cd $script_dir
# vsim +permissive -t 1fs -sdfmax /users/micas/shares/project_HeMAiAv2/HeMAiA_2K_3D/target/sim/testharness/hemaia_mapped.sdf -sdfreport=$script_dir/../sdf_report.txt -sdfnoerror -voptargs=+acc -do "log -r /*" -warning 8386 -work ../work-vsim \
