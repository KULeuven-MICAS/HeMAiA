...
#define true 1
#define false 0
// We only initialize the first 16KB and last 16KB of the TCDM
...
void bootrom() {
    ...
    for (uint32_t i = 0; i < N_CLUSTERS_PER_CHIPLET; i++) {
        ...
        // Last 16KB
        sys_dma_blk_memcpy(
            chiplet_addr_transform(QUAD_WIDE_CLUSTER_0_TCDM_BASE_ADDR + i * CLUSTER_TCDM_SIZE + CLUSTER_TCDM_SIZE - INIT_TCDM_SIZE),
            chiplet_addr_transform(0x1000000000ULL + CLUSTER_TCDM_SIZE - INIT_TCDM_SIZE),
            INIT_TCDM_SIZE);
        }        
